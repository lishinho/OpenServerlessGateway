...
/**
 * ServiceComb协议适配器
 * 支持 ServiceComb Highway/REST 协议调用
 */
public class ServiceCombProtocolAdapter implements ProtocolAdapter {

    private static final Logger log = LoggerFactory.getLogger(ServiceCombProtocolAdapter.class);

    private static final String NAME = "ServiceCombProtocolAdapter";
    private static final String TYPE = "servicecomb";

    private String registryAddress;
    private int defaultTimeout;
    private final Map<String, Object> serviceCache = new ConcurrentHashMap<>();

    private Class<?> referenceConfigClass;
    private Class<?> microserviceConfigClass;
    ...
    public void init(Map<String, Object> config) {
        ...
        try {
            referenceConfigClass = Class.forName("org.apache.servicecomb.config.ReferenceConfig");
            microserviceConfigClass = Class.forName("org.apache.servicecomb.config.MicroserviceConfig");
            log.info("初始化ServiceComb协议适配器: registry={}", registryAddress);
        } catch (ClassNotFoundException e) {
            log.warn("ServiceComb 依赖未找到，ServiceComb适配器将使用模拟模式");
        }
    }
    ...
    public Object invoke(GatewayContext context, ApiMetadata apiMetadata, Map<String, Object> params) {
        ...
        try {
            ...
            log.debug("ServiceComb调用: service={}, method={}, params={}", 
            ...
            log.debug("ServiceComb调用成功: service={}, method={}", serviceName, methodName);
            return result;
        } catch (Exception e) {
            log.error("ServiceComb调用失败: service={}, method={}, error={}", 
                serviceName, methodName, e.getMessage(), e);
            throw new ProtocolConvertException("ServiceComb调用失败: " + e.getMessage(), e);
        }
    }
    ...
    private Object getOrCreateService(String serviceName, String version, String group, int timeout) {
        ...
        return serviceCache.computeIfAbsent(cacheKey, key -> {
            try {
                if (referenceConfigClass != null) {
                    return createServiceCombReference(serviceName, version, group, timeout);
                } else {
                    throw new ProtocolConvertException("ServiceComb 依赖未引入，无法创建服务引用");
                }
            } catch (Exception e) {
                log.error("创建ServiceComb服务引用失败: {}", e.getMessage());
                throw new ProtocolConvertException("创建ServiceComb服务引用失败: " + e.getMessage(), e);
            }
        });
    }
     ...
     * 创建ServiceComb服务引用
     */
    private Object createServiceCombReference(String serviceName, String version, String group, int timeout) throws Exception {
        Object referenceConfig = referenceConfigClass.getDeclaredConstructor().newInstance();
        
        referenceConfigClass.getMethod("setMicroserviceName", String.class)
            .invoke(referenceConfig, serviceName);
        
        if (version != null && !version.isEmpty()) {
            referenceConfigClass.getMethod("setVersion", String.class).invoke(referenceConfig, version);
        }
        if (registryAddress != null && !registryAddress.isEmpty()) {
            referenceConfigClass.getMethod("setRegistryAddress", String.class)
                .invoke(referenceConfig, registryAddress);
        }
        
        log.info("创建ServiceComb服务引用: service={}, version={}", serviceName, version);
        
        Method getMethod = referenceConfigClass.getMethod("get");
